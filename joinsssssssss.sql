...
inner join developer_dersler B on A.ders_id=B.ders_id;
...
Left join aktorler B On A.film_id=B.film_id;

select film_ismi,kategori,aktor_ismi from aktorler A
right join filmler B On A.film_id=B.film_id;

-- SORU2: Tüm actor_name'leri ve bu aktorlerin oynadıgı film_name'lerini listeleyiniz.
 select aktor_ismi,film_ismi from aktorler A left join filmler B
 On  A.film_id=B.film_id;

 select aktor_ismi,film_ismi from filmler A right join aktorler B
 On  A.film_id=B.film_id;
...
select film_ismi,aktor_ismi from filmler A full join aktorler B
on A.film_id=B.film_id;
