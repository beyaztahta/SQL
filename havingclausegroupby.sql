--where satıları filtreler.
--aggregate methodları ile kullanılamaz.
--group by dan once kullanılır.
--select update delete ile kullanılır.


--having ise grupları filtreler.
--Aggregate methodları ile kullanılır.
--group by da sonra kullanılır.
--sadece select ile kullanılır.
...
--4.cinsiyet ve ulkelere gore maas ort 4500 den cok olanlar
...
--6.çalışan sayısı 1’den fazla olan şehirleri ve çalışan sayılarını getiren sorguyu yazalım

select sehir,count(*)from kisiler group by sehir having count(sehir)>1

--7.Maaş ortalaması 3000’den fazla olan ülkelerdeki erkek çalışanların maaş ortalaması.
select ulke,round(avg(maas))from kisiler where cinsiyet='E'group by ulke having avg(maas)>3000;

--8.Erkek çalışanların sayısı 1’den fazla olan ülkelerin maaş ortalamasını getiren sorgu:
select ulke,round(avg(maas))from kisiler where cinsiyet='E'group by ulke having count(*)>1
